...
# ── Shifokor ish jadvali (working_hours) ────────────────────────────
# Frontend endi bu qiymatni erkin matn sifatida emas, balki kun
# checkbox'lari + ikkita <input type="time"> orqali quradi (base.html,
# composeWorkingHours()), shuning uchun backend ham qat'iy grammatikani
# talab qilishi mumkin — bu reminder_service._parse_working_hours()
# doim to'g'ri formatlangan qiymat bilan ishlashini kafolatlaydi.
# Format: "<kunlar> H:MM-H:MM", masalan "Du-Ju 09:00-18:00" yoki
# "Du, Chor, Juma 09:00-18:00". Kunlar qismi ixtiyoriy.
_WORKING_HOURS_DAY = r"(?:Du|Se|Chor|Pay|Ju|Sha|Yak)"
WORKING_HOURS_PATTERN = re.compile(
    r"^(?:" + _WORKING_HOURS_DAY + r"(?:-" + _WORKING_HOURS_DAY + r")?"
    r"(?:,\s*" + _WORKING_HOURS_DAY + r"(?:-" + _WORKING_HOURS_DAY + r")?)*\s+)?"
    r"([01]?\d|2[0-3]):([0-5]\d)-([01]?\d|2[0-3]):([0-5]\d)$"
)


def _check_working_hours_format(v: str) -> str:
    """Ikkala validator uchun umumiy format tekshiruvi: qat'iy
    grammatikaga (kunlar + H:MM-H:MM) mos kelishi va tugash vaqti
    boshlanish vaqtidan keyin bo'lishi shart."""
    m = WORKING_HOURS_PATTERN.match(v)
    if not m:
        raise ValueError(
            "Ish jadvali noto'g'ri formatda — kunlar va vaqt "
            "tanlagichlari orqali kiriting (masalan: Du-Ju 09:00-18:00)"
        )
...
def _validate_working_hours(v: Optional[str]) -> Optional[str]:
    """DoctorBase.working_hours uchun — ixtiyoriy maydon, bo'sh
    qiymatga ruxsat beradi (shifokorning ish jadvali hali
    belgilanmagan bo'lishi mumkin)."""
    if v is None or v.strip() == "":
        return None
    return _check_working_hours_format(v.strip())


def _validate_required_working_hours(v: str) -> str:
    """ClinicSettingsUpdate.working_hours uchun — klinikaning umumiy
    ish vaqti majburiy maydon, shuning uchun bo'sh qiymatga ruxsat
    berilmaydi."""
    v = (v or "").strip()
    if not v:
        raise ValueError("Ish jadvali kiritilishi shart")
    return _check_working_hours_format(v)
...
class DoctorBase(BaseModel):
    ...
    @classmethod
    def _validate_specialty(cls, v: str) -> str:
        if v not in DOCTOR_SPECIALTIES:
            raise ValueError(
                f"Noto'g'ri mutaxassislik — ruxsat etilganlar: {', '.join(DOCTOR_SPECIALTIES)}"
            )
        return v

    @field_validator("working_hours")
    @classmethod
    def _validate_doctor_working_hours(cls, v: Optional[str]) -> Optional[str]:
...
class AdminProfileUpdate(BaseModel):
    ...
    @field_validator("working_hours")
    @classmethod
    def _validate_admin_profile_working_hours(cls, v: Optional[str]) -> Optional[str]:
        # AdminProfileSettings.working_hours ustuni ClinicSettingsUpdate
        # (settings.html) bilan BIR XIL — hozircha profile.html'da bu
        # maydon uchun input yo'q, lekin API darajasida ham bir xil
        # qat'iy formatni talab qilib, kelajakda shu yo'l bilan ham
        # noto'g'ri qiymat yozilib qolishining oldini olamiz.
        return _validate_working_hours(v)
...
# 🆕 Oldindan tanilgan davlat integratsiyasi provayderlari — frontend
# (templates/profile.html) buni erkin `<input type="text">` o'rniga
# `<select>` sifatida ko'rsatadi ("Boshqa" tanlansa, alohida qo'lda
# kiritish maydoni ochiladi). Ro'yxat GovIntegrationSettingsResponse
# orqali ham qaytariladi (known_providers) — shunday qilib frontend
# hech qachon backend validatsiyasidan uzilib qolmaydi (ikkalasi ham
# shu yagona ro'yxatdan foydalanadi).
GOV_INTEGRATION_PROVIDERS: List[str] = ["OneID", "MyGov"]
GOV_INTEGRATION_CUSTOM_LABEL = "Boshqa"
...
class GovIntegrationSettingsBase(BaseModel):
    ...
    @classmethod
    def _normalize_integration_name(cls, value: Optional[str]) -> Optional[str]:
        """Erkin matn o'rniga select ishlatilgani uchun endi asosiy
        oqim bilan ("OneID", "MyGov") kelgan qiymat aniq (case-sensitive)
        moslashadi — lekin baribir case-insensitive solishtirib,
        yozilish farqini ("oneid", "One Id" va h.k.) kanonik shaklga
        ("OneID") normallashtiramiz. "Boshqa" orqali kiritilgan haqiqiy
        maxsus nom (ro'yxatda yo'q) o'zgarishsiz saqlanadi — bu yerda
        cheklangan ro'yxat majburiy emas, faqat ma'lum provayderlar
        uchun izchillik ta'minlanadi."""
        if value is None:
            return value
        stripped = value.strip()
        if not stripped:
            return None
        for known in GOV_INTEGRATION_PROVIDERS:
...
class GovIntegrationSettingsResponse(GovIntegrationSettingsBase):
    ...
    # 🆕 Frontend select'ni backend validatsiyasi bilan sinxron ushlab
    # turish uchun (ikkalasi ham shu ro'yxatdan foydalanadi).
    known_providers: List[str] = Field(default_factory=lambda: list(GOV_INTEGRATION_PROVIDERS))
    # 🆕 Oxirgi "Ulanishni tekshirish" natijasi.
...
class GovIntegrationTestResponse(BaseModel):
    """POST /api/admin/gov-integration/test-connection javobi. Faqat
    ulanish (tarmoq darajasida) tekshiriladi — hech qanday bemor
    ma'lumoti yuborilmaydi/qaytarilmaydi."""
    success: bool
    message: str
...
class ClinicSettingsUpdate(BaseModel):
    ...
    @field_validator("working_hours")
    @classmethod
    def _validate_clinic_working_hours(cls, v: str) -> str:
        return _validate_required_working_hours(v)
