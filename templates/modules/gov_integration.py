...
@admin_router.post("/test-connection", response_model=schemas.GovIntegrationTestResponse)
def test_gov_integration_connection(
    db: Session = Depends(get_db),
    user: models.User = Depends(get_current_user),
) -> schemas.GovIntegrationTestResponse:
    """🆕 "Ulanishni tekshirish" — admin sozlagan davlat tizimi API
    manziliga YENGIL (tarmoq darajasidagi) so'rov yuboradi va serverning
    umuman javob berish-bermasligini tekshiradi.

    MUHIM: bu haqiqiy PINFL/pasport tekshiruvi EMAS — hech qanday bemor
    ma'lumoti yuborilmaydi. Maqsad — admin "Saqlash"ni bosgandan keyin
    darhol bemor kartochkasiga o'tmasdan turib, manzil/kalit to'g'ri
    kiritilganini (server umuman ulanishga tayyorligini) bilishi.
    Har qanday HTTP javob (401/404 bo'lsa ham) "ulanish bor" deb
    hisoblanadi — biz faqat tarmoq/SSL darajasini tekshiramiz,
    avtorizatsiya yoki endpoint to'g'riligini emas (buning uchun
    haqiqiy /verify chaqiruvi kerak bo'ladi)."""
    settings = _get_settings(db)
    tested_at = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)

    if not settings.is_enabled:
        success = False
        message = "Integratsiya o'chirilgan (mock rejimda ishlayapti) — real ulanish tekshirilmadi."
    elif not (settings.api_url and settings.api_key):
        success = False
        message = "API manzili yoki kalit to'liq kiritilmagan — avval sozlamalarni saqlang."
    else:
        try:
            resp = httpx.get(settings.api_url, timeout=8.0)
            success = True
            message = f"Server javob berdi (HTTP {resp.status_code}). Tarmoq ulanishi mavjud."
        except httpx.HTTPError as exc:
            success = False
            message = f"Ulanib bo'lmadi: {type(exc).__name__}"
            logger.error(
                "Gov integration (%s) test-connection muvaffaqiyatsiz: %s",
                settings.integration_name, type(exc).__name__,
            )

    settings.last_test_status = "success" if success else "failed"
    settings.last_tested_at = tested_at
    db.commit()

    # PII yo'q — faqat natija (muvaffaqiyatli/muvaffaqiyatsiz).
    log_action(
        db, user, "gov_integration.test_connection", "GovIntegrationSettings", settings.id,
        f"success={success}",
    )
    return schemas.GovIntegrationTestResponse(success=success, message=message, tested_at=tested_at)
