...
def list_allergy_substances(db: Session = Depends(get_db)) -> List[str]:
    """Barcha bemorlarda qayd etilgan NOYOB allergiya moddalari nomlari —
    frontendda `<datalist>` (autocomplete) uchun ishlatiladi (4-band,
    O'RTA muhimlik): #al-substance hali ham erkin matn, lekin admin/
    reception mavjud yozuvlardan birini tanlasa, "Penitsillin"/
    "penicillin"/"Pеnisillin" kabi yozilish farqlari kamayadi va
    ?allergy=<matn> (ILIKE) qidiruvi izchil ishlaydi.

    Ruxsat: list_patients() dagi allergy/chronic_condition filtri bilan
    bir xil (admin/reception/doctor) — bu ro'yxat ham tibbiy ma'lumot
    (klinikada qanday allergiyalar qayd etilgani) oshkor qiladi."""
    rows = (
        db.query(models.Allergy.substance)
        .filter(models.Allergy.substance.isnot(None))
        .distinct()
        .order_by(models.Allergy.substance.asc())
        .all()
    )
...
def list_chronic_condition_names(db: Session = Depends(get_db)) -> List[str]:
    """Xuddi list_allergy_substances() kabi, lekin
    ChronicCondition.name uchun — #cc-name maydonidagi autocomplete."""
    rows = (
        db.query(models.ChronicCondition.name)
        .filter(models.ChronicCondition.name.isnot(None))
        .distinct()
        .order_by(models.ChronicCondition.name.asc())
        .all()
    )
    return [r[0] for r in rows if r[0]]
...
# 🔐 KRITIK TUZATISH (#1-band): ilgari bu ikkita GET faqat login talab
# qilardi (router darajasidagi Depends(get_current_user)), require_role
# yo'q edi — istalgan xodim (jumladan cashier) PINFL/pasport/telefon va
# moliyaviy (financials) ma'lumotni ko'ra olardi. Fayldagi qolgan barcha
# tibbiy/shaxsiy sub-resurslar (allergiya, surunkali kasallik, davolanish
# tarixi, palatalar) bilan bir xil ruxsat matritsasiga moslashtirildi:
# faqat admin/reception/doctor. cashier'ga to'lov bilan bog'liq narsalar
# uchun payments.py'dagi maxsus (PINFL/passport ko'rsatmaydigan)
# endpointlar yetarli — u yerga alohida require_role bilan ruxsat
# berilgan, bu yerga emas.
@router.get(
    "/{patient_id}",
    response_model=schemas.PatientDetail,
    dependencies=[Depends(require_role("admin", "reception", "doctor"))],
)
def get_patient(
    patient_id: int,
    ...
    # 🔒 lab_doctor faqat O'ZIGA biriktirilgan bemorni ko'ra oladi —
    # main.py::patient_detail_page (HTML) dagi bilan bir xil qoida,
    # bu yerda ham qo'llanadi (to'g'ridan-to'g'ri API chaqiruvi orqali
...
def upload_patient_photo(
    ...
    🐛 FIX: ilgari `async def` bo'lib, ichida sinxron `db.commit()`,
    `db.refresh()` va Pillow (`Image.open/.thumbnail/.save`) kabi
    BLOKLOVCHI operatsiyalarni chaqirardi — bu FastAPI event loop'ini
    to'sib qo'yardi va parallel so'rovlarni sekinlashtirardi. Endi
    oddiy (sinxron) `def` — FastAPI bunday endpointlarni avtomatik
    ravishda alohida threadpool'da ishga tushiradi, shuning uchun
    barcha bloklovchi kod (fayl o'qish, rasm qayta ishlash, DB yozish)
    event loop'dan tashqarida bajariladi.
    """
    patient = _get_patient_or_404(db, patient_id)
    ...
    # `await file.read()` o'rniga sinxron `.file.read()` — endi butun
    # funksiya sinxron, shuning uchun `await` shart emas (va event
    # loop'da emas, threadpool'da ishlaydi).
    raw = file.file.read()
    if not raw:
