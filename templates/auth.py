...
def get_user_cached(username: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if user:
            # 🔒 password_hash bu yerga QO'SHILMAYDI — Redis (yoki
            # boshqa worker xotirasi) buzilsa ham parol xeshlari
            # oshkor bo'lmasligi uchun. Kesh faqat username→id/rol
            # kabi nozik bo'lmagan maydonlarni tezlashtirish uchun
            # ishlatiladi; parolni tekshirish har doim bazadan olingan
            # `user.password_hash` orqali amalga oshiriladi (pastda,
            # modules/auth_module.py'da).
            user_data = {
                "id": user.id,
